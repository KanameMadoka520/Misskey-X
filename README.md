...
- 两个专用频道通过频道发帖权限配置限制为仅频道管理员发帖，普通用户可以阅读但不能发布。
- 后端对频道发帖权限做统一硬校验，即使绕过前端直接调用 API，没有权限的用户也会被拒绝。
- 频道发帖权限校验失败会返回明确的 `CANNOT_POST_TO_RESTRICTED_CHANNEL` 错误码。

### 帖子阅读与详情交互

- 调高了帖子自动折叠阈值，长文字和多图帖子不再过早只显示很短一段。
- URL 预览支持 `summary_large_image` 风格的大图卡片，让带大图封面的链接更接近 X/Twitter 的展示效果。
- 大图 URL 预览只在单个链接且帖子未附带媒体时启用；多个链接、已有图片/文件、悬浮预览和聊天中的混合内容会自动保持紧凑卡片，避免信息流被大图刷屏。
- 大图 URL 预览使用服务端生成的静态缩略图，不直接加载原始大图或动图，降低超大 OGP 图片和 GIF 对流量及性能的影响。
- 用户可在偏好设置中启用“始终以紧凑样式显示 URL 预览”，按个人习惯关闭大图卡片。
- 保留点击时间戳进入帖子详情的原有方式。
...
- 新增登录用户专属的“帖子瀑布流”页面，左侧导航入口固定显示在“发现”上方；已有自定义导航配置的用户也无需手动重置菜单。
- 页面以响应式瀑布流浏览站内近期帖子，桌面端可在双栏和三栏之间切换，窄屏自动降为单栏。
- 支持随机排布、按最近时间、有图片、无图片、指定用户、已关注作者和未关注作者等筛选。
- 支持按频道关系筛选：只展示已关注频道、展示所有频道、不展示已关注频道内容、不展示频道内容。
- 随机模式仅从最近 500 条符合条件的候选帖子中按种子稳定洗牌，不执行高负担的数据库全表随机排序。
- 瀑布流复用 Misskey 原有帖子组件以及可见性、静音、屏蔽和转发静音规则，保留回复、转发、表情回应和详情等完整交互。

### 受控代理发帖

- 新增角色策略“允许代替其他用户发帖”，默认关闭，只能由有权管理角色的管理员配置。
- 角色编辑器将该策略明确标记为“Misskey-X 独有设置”，并解释低、中、高优先级在多角色冲突时的计算方式。
- 角色表单会同步提交最新开关状态，并等待服务端确认保存成功后再离开编辑页，避免刚修改的策略回落到“使用基本角色的值”。
- 获得授权的账号可在发帖器中搜索本地用户并以目标用户身份发布，也可为普通原创帖子设置不晚于当前时间的过去时间。
- 代理发帖只允许使用有效的本地账号；过去时间不能早于目标账号创建时间，也不能用于回复或转发。
- 代理帖强制仅本站发布，并继续按照目标作者的频道权限、公开发帖权限、屏蔽关系和提及上限执行校验。
- 代理上传的附件直接归属于目标用户网盘，使用目标用户的网盘容量、文件大小和文件类型策略，并统一存放到目标网盘根目录。
- 操作者自己网盘中的文件不能直接附加到目标用户的代理帖，避免附件所有权与帖子作者不一致。
- 切换或取消代理身份时会中止未完成上传并清空当前附件，防止不同用户的附件在同一次发帖中混用。
- 代理发帖和向目标用户网盘上传文件都会写入管理日志，保留实际操作者、目标用户、帖子或文件 ID 以及帖子时间。
...
- 频道新增动态发帖权限：允许任何人发帖、仅频道管理员发帖、仅频道管理员和协作者发帖。
- 频道协作者只用于判断是否可以在受限频道中发帖，不拥有其他频道管理权限。
- 社区管理员权限高于频道权限限制，可以在任意频道发帖，并可在频道编辑页调整频道管理员和协作者。
- 频道页面新增“用户”标签页，可查看频道管理员、协作者和已关注本频道的用户。
