...
export function genOpenapiSpec(config: Config, includeSelfRef = false) {
	const spec = {
		...
		externalDocs: {
			...
			url: 'https://github.com/KanameMadoka520/Misskey-X',
		},
	...
	for (const endpoint of copiedEndpoints) {
		...
		const info = {
			...
			externalDocs: {
				...
				url: `https://github.com/KanameMadoka520/Misskey-X/blob/main/packages/backend/src/server/api/endpoints/${endpoint.name}.ts`,
			},
			...(endpoint.meta.tags ? {
